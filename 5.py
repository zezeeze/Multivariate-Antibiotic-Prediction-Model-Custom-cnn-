...
def load_data(image_folder, label_file):
    ...
    for index, row in labels_df.iterrows():
        ...
        if image is not None:
            ...
            images.append(image)
            labels.append([row['A'], row['B'], row['C'], row['D'], row['E']])
        else:
...
# 数据文件夹路径
image_folder = r'E:\Ky\documents\xl\5y\Total4'
label_file = r'E:\Ky\documents\xl\5y\labels4.xlsx'
...
# 使用ImageDataGenerator进行批处理
datagen = ImageDataGenerator()

# 生成训练数据的生成器
train_generator = datagen.flow(X_train, y_train, batch_size=16)  # 设置批处理大小为32
...
# 保存模型
save_path = r'E:\Ky\documents\xl\5y\5y.keras'
...
# 计算每种抗生素浓度的评估指标
results = {}
for i, antibiotic in enumerate(['A', 'B', 'C', 'D', 'E']):
...
plt.scatter(y_train[:, 3], y_train_pred[:, 3], label='抗生素 D', alpha=0.5)
plt.scatter(y_train[:, 4], y_train_pred[:, 4], label='抗生素 E', alpha=0.5)
...
plt.scatter(y_test[:, 3], y_pred[:, 3], label='抗生素 D', alpha=0.5)
plt.scatter(y_test[:, 4], y_pred[:, 4], label='抗生素 E', alpha=0.5)
...
# 定义保存路径
output_file = r'E:\Ky\documents\xl\5y\results\wh\results2.xlsx'
